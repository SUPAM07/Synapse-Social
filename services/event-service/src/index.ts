...
const start = async (): Promise<void> => {
  try { await kafkaPublisher.connect(); } catch (err) { logger.warn('Kafka connection failed', { err }); }
  app.listen(config.port, () => logger.info(`Event service running on port ${config.port}`));
};
